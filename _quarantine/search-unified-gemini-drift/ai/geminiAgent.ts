...
/**
 * Shape of the JSON the backend will send into Gemini.
 * You can expand these later, but this is enough to start.
 */
export type LitGeminiInput = {
  company_profile?: unknown;
  lit_search_context?: unknown;
  command_center_state?: unknown;
  user_goal?: string;
};
  ...
  responseSchema: {
    ...
    properties: {
      normalized_company: {
        type: Type.OBJECT,
        description:
          "Canonical company profile used by LIT Search and Command Center.",
        properties: {
 ...
 */
export async function runGeminiAgent(input: LitGeminiInput) {
  ...
  // Use the Gemini 2.5 Flash model you saw in the models list.
