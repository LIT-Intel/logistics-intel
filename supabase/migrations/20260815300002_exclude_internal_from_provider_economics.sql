-- Provider economics summary: exclude internal/test identities from
-- active_paid_customers / cost-per-customer / MRR. Exact prior definition with
-- only `AND NOT public.is_internal_user(s.user_id)` added to the two
...
begin
  ...
  where s.status = 'active'
    ...
    and not public.is_internal_user(s.user_id);   -- exclude internal/test identities
