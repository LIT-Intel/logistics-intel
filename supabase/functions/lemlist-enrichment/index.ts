...
//
// Used by enrich-contact-orchestrator as the Lemlist fallback provider.
// Lemlist enrichment is asynchronous: this function submits up to 500
// enrichment requests, records lit_contact_enrichment_jobs, and returns
// pending=true with provider request IDs. Results are fetched later by
// lemlist-enrichment-sync or by webhook.
...
function workflowsFor(body: RequestBody, target: Target): string[] {
  ...
  for (const w of requested) {
    if (["find_email", "find_phone", "verify", "linkedin_enrichment"].includes(w)) out.add(w);
  }
  if (body.reveal_phone_number || target.linkedin_url) out.add("find_phone");
...
Deno.serve(async (req: Request) => {
  ...
  if (!validRequests.length) {
    return json({ ok: true, provider: "lemlist", pending: false, contacts: [], count: 0, exhausted: true, errors: requests.map((r) => ({ target: r.target.id || r.target.email || r.target.linkedin_url, error: "insufficient_input" })) });
  }

  const payload = validRequests.map((r) => ({
    input: r.input,
    enrichmentRequests: r.workflows,
    metadata: r.metadata,
  }));
