// Shared mail-merge layer for the email dispatcher and any future
// channel that emits text to a recipient.
//
// Token format: `{{name}}` with optional inner whitespace. Names are
// case-insensitive on lookup. Missing values can either leave the
// literal token in place (default) or be replaced with a configured
// fallback string.
//
// Mirrored almost verbatim by frontend/src/lib/mergeVars.ts so what the
// preview renders matches what the dispatcher sends.
...
function readVar(ctx: MergeContext, key: string): unknown {
  ...
  // Fall back to case-insensitive search to be forgiving — templates
  // saved with {{First_Name}} should still resolve {first_name} context.
...
function stringify(value: unknown): string {
  ...
  // Don't dump `[object Object]` into the body.
  return "";
}
...
/** List variable names referenced by the template (deduped, lower-cased). */
...
/** Names referenced by the template that the context cannot resolve. */
...
/**
 * Build the canonical merge context for a recipient. Edge functions and
 * frontend share this shape so a previewed template renders the same as
 * what gets sent.
 */
...
export function buildMergeContext(args: {
  ...
  const ctx: MergeContext = {
    ...
    // Free-form extras win over the structured ones if a custom key matches.
