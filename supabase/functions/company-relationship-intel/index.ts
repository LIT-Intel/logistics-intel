...
(b) Warehouse / distribution-center / plant / HQ / store locations. Prioritize finding DISTRIBUTION CENTERS and WAREHOUSES (kind=dc/warehouse), not just HQ and plants — run searches like "<company> distribution center", "<company> warehouse locations", and site:<company domain> locations/careers pages. Facilities MUST come from PRIMARY/AUTHORITATIVE sources ONLY: the company's own domain (any subdomain — locations pages, contact pages, careers/job postings, dealer/store locators, press releases it published), SEC filings (sec.gov), the company's careers/locations pages, or official government/port-authority records. Industrial real-estate press (e.g. bisnow.com, rebusinessonline.com, CoStar news) is acceptable ONLY when the article names a specific address or city for the company's own facility. General stock-info sites, wikis, and AI-content aggregators (e.g. stockanalysis.com, ventureatlas.org) are NOT acceptable facility sources — OMIT a facility rather than cite one of them.
...
- Every facility's source_url and every relationship's source_url must point to a page that satisfies the primary-source rules in (b). sources[] should list every domain you consulted during research, but a URL that fails the facility-source bar must NEVER appear as a facility's or relationship's source_url.
