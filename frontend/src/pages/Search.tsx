...
// App wiring: keep the hard search behavior
import { useSearch } from "@/app/search/useSearch";
import {
  getFilterOptions,
  getFilterOptionsOnce,
  saveCompanyToCrm,
  getCompanyKey,
  searchCompanies as searchCompaniesApi, // <- use gateway-safe client
} from "@/lib/api";
...
function ResultKPI({
  ...
  return (
    <div className="p-3 border border-gray-200 rounded-xl bg-white text-center min-h-[96px] flex flex-col items-center justify-center w-full overflow-hidden">
      ...
      <div className="text-xl font-bold text-gray-900 truncate w-full max-w-full" title={String(value ?? "—")}>
        {value ?? "—"}
      </div>
      <div className="text-[11px] uppercase text-gray-500 font-medium mt-1 truncate w-full max-w-full" title={label}>
        {label}
      </div>
    </div>
  );
...
function SaveToCommandCenterButton({
  ...
  const onClick = async () => {
    ...
    const allowed =
      email === "vraymond@logisticintel.com" || email === "support@logisticintel.com" || hasFeature("crm");
    if (!allowed) {
      setShowUpsell(true);
      return;
    }
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="mt-4 flex justify-end gap-2">
                  <button className="px-3 py-2 text-sm rounded-lg text-gray-700 hover:bg-gray-100" onClick={() => setShowUpsell(false)}>
                    Not right now
                  </button>
                  <button
        ...
        <thead>
          <tr>
            {["Company", "Shipments (12m)", "Last Activity", "Top Route", "Actions"].map((col) => (
              <th key={col} className="px-6 py-3 text-left text-xs font-medium text-gray-600 uppercase tracking-wider bg-gray-50">
                {col}
              </th>
            ))}
          </tr>
...
export default function SearchPage() {
  ...
  // Debounced suggestions via gateway-safe client
  const debouncedSuggest = useDebouncedCallback(async (query: string) => {
    ...
    try {
      const data = await searchCompaniesApi({ q: str, page: 1, pageSize: 10 });
      // supports either { results } or array
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center justify-between">
                          ...
                          {s.domain && <span className="ml-2 text-xs text-gray-500 truncate">{s.domain}</span>}
                        </div>
                      </button>
                    );
                  })}
