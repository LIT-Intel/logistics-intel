  ...
  return (
    <section>
      ...
      <p className="mt-2 text-gray-600 max-w-2xl">
        Purpose-built for logistics sales and marketing with the freshest shipment signals.
      </p>
      <div className="mt-8 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
        {FEATURES.map((feature) => (
          <div key={feature.title} className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
            <div className={`inline-flex items-center rounded-md px-2 py-1 text-sm ${feature.color}`}>{feature.title}</div>
