...
interface Company { id: string; name: string; domain?: string; shipments12m?: number; lastActivity?: string; topRoutes?: Array<{ origin: string; dest: string; count: number }>; topCarriers?: Array<{ name: string; sharePct: number }>; }
interface Contact { id: string; name: string; title?: string; email?: string; phone?: string; source?: string; linkedinUrl?: string; avatarUrl?: string; isPrimary?: boolean; status?: 'active'|'archived'|'invalid'; }

const demoCompany: Company = { id: 'C-982174', name: 'Acme Robotics, Inc.', domain: 'acmerobotics.com', shipments12m: 312, lastActivity: '2025-09-28', topRoutes: [ { origin: 'CN SHA', dest: 'US LAX', count: 128 }, { origin: 'TW TPE', dest: 'US SFO', count: 66 }, { origin: 'DE FRA', dest: 'US JFK', count: 41 } ], topCarriers: [ { name: 'EVA Air', sharePct: 28 }, { name: 'Lufthansa Cargo', sharePct: 19 }, { name: 'China Eastern', sharePct: 13 } ] };
const demoContacts: Contact[] = [ { id: 'p1', name: 'Maya Chen', title: 'VP Supply Chain', email: 'maya@acmerobotics.com', phone: '+1 (415) 555-0142', source: 'LIT Enrich', linkedinUrl: 'https://www.linkedin.com/in/mayachen', avatarUrl: 'https://i.pravatar.cc/64?img=1', isPrimary: true, status: 'active' }, { id: 'p2', name: 'Derrick Cole', title: 'Logistics Manager', email: 'derrick@acmerobotics.com', phone: '+1 (949) 555-8821', source: 'LIT Enrich', linkedinUrl: 'https://www.linkedin.com/in/derrickcole', avatarUrl: 'https://i.pravatar.cc/64?img=2', status: 'active' }, { id: 'p3', name: 'Samir Patel', title: 'Procurement Lead', email: 'samir@acmerobotics.com', phone: '+1 (212) 555-3007', source: 'Apollo Enrich', linkedinUrl: 'https://www.linkedin.com/in/samirpatel', avatarUrl: 'https://i.pravatar.cc/64?img=3', status: 'active' } ];
...
export default function LITEnrichCommandCenterDemo() {
  ...
  // Handlers wired per spec
  async function handleEnrichCompany() {
    setLoading(true);
    log(`⏳ ${new Date().toLocaleString()} — Enrichment started for ${company.name}`);
    const r = await fetch(`${CRM_BASE}/enrich`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ company_id: company.id, domain: company.domain }) });
    const ok = r.ok;
  ...
  async function handleFindContacts() {
    ...
    const r = await fetch(`${CRM_BASE}/contacts?domain=${encodeURIComponent(company.domain ?? '')}&limit=10`);
  ...
  async function handleGenerateBrief() {
    ...
    const r = await fetch(`${CRM_BASE}/briefing`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ company_id: company.id }) });
  ...
  async function handleSaveToCampaign(contactId: string) {
    ...
    const r = await fetch(`${CRM_BASE}/campaigns/addContact`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ contact_id: contactId, company_id: company.id }) });
    log(`${r.ok ? '✅' : '⚠️'} ${new Date().toLocaleString()} — Contact ${contactId} ${r.ok ? 'saved to' : 'failed for'} campaign`);
  }
  ...
  async function handleAddContactManual() {
    ...
    const r = await fetch(`${CRM_BASE}/contacts/enrich`, {
      method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({
        company_id: company.id, domain: company.domain, name: newContact.name, title: newContact.title, email: newContact.email, phone: newContact.phone, linkedinUrl: newContact.linkedinUrl
      })
    });
  ...
  async function handleExportContactsPdf() {
    ...
    const r = await fetch(`${CRM_BASE}/contacts/export`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ company: { id: company.id, name: company.name, domain: company.domain }, contacts }) });
    if (!r.ok) { log(`⚠️ ${new Date().toLocaleString()} — PDF export failed`); return; }
    const blob = await r.blob();
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `${company.name.replace(/\s+/g,'_')}_contacts.pdf`;
    a.click();
  ...
  return (
    <div className="min-h-screen w-full bg-gradient-to-b from-slate-50 to-white px-[5px]">
        ...
        <div className="mx-auto max-w-7xl px-4 py-3 flex items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            <div className="hidden md:flex items-center gap-2 ml-4">
              <span className="inline-flex items-center gap-1 rounded-full bg-emerald-50 text-emerald-700 text-[11px] px-2 py-1 border border-emerald-200"><IconShield className="w-3 h-3" /> LIT Enrich Connected</span>
              <span className="inline-flex items-center rounded-full bg-gray-100 text-gray-700 text-xs px-2 py-1">Plan: Enterprise</span>
          ...
          <div className="flex items-center gap-2 w-full md:w-96">
            ...
            <button onClick={handleEnrichCompany} disabled={loading} className="inline-flex items-center gap-2 px-3 py-2 rounded-xl text-sm font-medium bg-indigo-600 text-white hover:bg-indigo-700 disabled:opacity-60 shadow-sm">
      ...
      <main className="mx-auto max-w-7xl px-4 py-6">
        ...
        <section className="xl:col-span-1 space-y-4">
          <div className="p-5 rounded-2xl bg-white border border-slate-100 shadow-sm">
            ...
            <div className="grid grid-cols-3 gap-3 mt-4">
              ...
              <Stat label="Top Lane" value={company.topRoutes?.[0] ? `${company.topRoutes[0].origin} → ${company.topRoutes[0].dest}` : '—'} hint={`${company.topRoutes?.[0]?.count ?? 0} moves`} />
              <Stat label="Last Activity" value={company.lastActivity ?? '—'} />
            </div>
          </div>

          <div className="p-5 rounded-2xl bg-white border border-slate-100 shadow-sm">
            <div className="flex items-center justify-between mb-3"><div className="font-medium text-slate-900">Enrichment</div><span className="inline-flex items-center gap-1 rounded-full bg-emerald-50 text-emerald-700 text-[11px] px-2 py-1 border border-emerald-200"><IconShield className="w-3 h-3" /> LIT Enrich Active</span></div>
            <div className="space-y-2 text-sm text-slate-600">
              ...
              <div className="flex items-center gap-2">
                <button onClick={handleFindContacts} disabled={loading} className="px-3 py-2 rounded-xl text-sm font-medium bg-emerald-600 text-white hover:bg-emerald-700 disabled:opacity-60">Find Contacts via LIT Enrich</button>
                <button onClick={handleEnrichCompany} disabled={loading} className="px-3 py-2 rounded-xl text-sm font-medium bg-slate-100 text-slate-900 hover:bg-slate-200 disabled:opacity-60">Refresh Attributes</button>
              </div>
            </div>
          </div>

          <div className="p-5 rounded-2xl bg-white border border-slate-100 shadow-sm"><div className="font-medium text-slate-900 mb-3">Run Log</div><div className="space-y-2 max-h-60 overflow-auto pr-1">{logs.map((l,i)=> (<div key={i} className="text-xs text-slate-600 bg-slate-50 border border-slate-200 rounded-lg p-2">{l}</div>))}</div></div>
        </section>

        <section className="xl:col-span-1 space-y-4">
          <div className="p-5 rounded-2xl bg-white border border-slate-100 shadow-sm">
            <div className="flex items-center justify-between mb-4">
              <div><div className="text-xs text-slate-500">Contacts</div><div className="text-lg font-semibold text-slate-900">Decision Makers</div></div>
              <div className="flex items-center gap-2">
                <span className="inline-flex items-center rounded-full bg-gray-100 text-gray-700 text-xs px-2 py-1">{contacts.length} results</span>
                <button onClick={handleExportContactsPdf} disabled={loading} className="px-3 py-2 rounded-xl text-sm font-medium bg-slate-900 text-white hover:bg-black disabled:opacity-60">Export Contacts PDF</button>
                <button onClick={handleFindContacts} disabled={loading} className="px-3 py-2 rounded-xl text-sm font-medium bg-indigo-600 text-white hover:bg-indigo-700 disabled:opacity-60">+ Add from LIT Enrich</button>
              </div>
            </div>
                ...
                <tbody>
                  {contacts.map((c) => (
                    <tr key={c.id} className={`border-t border-slate-200 ${c.status==='archived' ? 'opacity-60' : ''}`}>
                      <td className="px-3 py-2 font-medium text-slate-900">
                        <div className="flex items-center gap-2">
                          ...
                          <div>
                            ...
                            {c.linkedinUrl && (<a href={c.linkedinUrl} target="_blank" className="inline-flex items-center gap-1 text-xs text-slate-600 hover:underline"><IconLinkedIn className="w-3 h-3"/> View LinkedIn</a>)}
                          </div>
                        </div>
                      </td>
                      ...
                      <td className="px-3 py-2"><span className="inline-flex items-center rounded-full bg-gray-100 text-gray-700 text-xs px-2 py-1">{c.source ?? '—'}</span></td>
                      <td className="px-3 py-2 text-right">
                        <div className="inline-flex items-center gap-2">
                          <button onClick={()=>{/* Save button retained as static; campaigns handled below */}} className="px-2 py-1 rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-900 inline-flex items-center gap-1"><IconSave className="w-4 h-4"/> Save</button>
                          <button onClick={()=>{ setPrimaryId(c.id); setContacts(prev => prev.map(x=> ({...x, isPrimary: x.id===c.id}))); log(`⭐ ${new Date().toLocaleString()} — Set primary contact → ${c.id}`); }} className="px-2 py-1 rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-900 inline-flex items-center gap-1"><IconStar className="w-4 h-4"/> Primary</button>
                          <a href={c.email ? `mailto:${c.email}` : undefined} className="px-2 py-1 rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-900 inline-flex items-center gap-1 disabled:opacity-60" aria-disabled={!c.email}><IconMail className="w-4 h-4"/> Email</a>
                          <a href={c.phone ? `tel:${c.phone}` : undefined} className="px-2 py-1 rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-900 inline-flex items-center gap-1 disabled:opacity-60" aria-disabled={!c.phone}><IconPhone className="w-4 h-4"/> Call</a>
                          <button onClick={()=>{ setContacts(prev => prev.map(x=> x.id===c.id ? {...x, status:'archived'} : x)); log(`📦 ${new Date().toLocaleString()} — Archived contact ${c.id}`); }} className="px-2 py-1 rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-900 inline-flex items-center gap-1"><IconArchive className="w-4 h-4"/> Archive</button>
                          <button onClick={()=>{ setContacts(prev => prev.filter(x=> x.id!==c.id)); log(`🗑️ ${new Date().toLocaleString()} — Deleted contact ${c.id}`); }} className="px-2 py-1 rounded-lg bg-slate-100 hover:bg-slate-200 text-rose-700 inline-flex items-center gap-1"><IconTrash className="w-4 h-4"/> Delete</button>
                          <button onClick={()=>handleSaveToCampaign(c.id)} className="hidden" aria-hidden="true" />
                        </div>
                      </td>
                    </tr>
                  ))}
          ...
          <div className="p-5 rounded-2xl bg-white border border-slate-100 shadow-sm">
            <div className="flex items-center justify-between mb-3"><div className="font-medium text-slate-900">Sequencing (read‑only demo)</div><span className="inline-flex items-center rounded-full bg-gray-100 text-gray-700 text-xs px-2 py-1">Campaigns</span></div>
            <div className="grid grid-cols-3 gap-3 text-sm">
              <div className="p-3 rounded-xl bg-slate-50 border border-slate-200"><div className="font-medium">Step 1</div><div className="text-slate-600">Intro email · Day 0</div></div>
              <div className="p-3 rounded-xl bg-slate-50 border border-slate-200"><div className="font-medium">Step 2</div><div className="text-slate-600">LinkedIn visit · Day 2</div></div>
              <div className="p-3 rounded-xl bg-slate-50 border border-slate-200"><div className="font-medium">Step 3</div><div className="text-slate-600">Call + VM · Day 4</div></div>
            </div>
          </div>
        </section>
        ...
        <section className="xl:col-span-1 space-y-4">
          <div className="p-5 rounded-2xl bg-white border border-slate-100 shadow-sm">
            <div className="flex items-center justify-between mb-4"><div><div className="text-xs text-slate-500">Briefing</div><div className="text-lg font-semibold text-slate-900">Pre‑Call Summary</div></div><button onClick={handleGenerateBrief} disabled={loading} className="px-3 py-2 rounded-xl text-sm font-medium bg-slate-900 text-white hover:bg-black disabled:opacity-60">Generate via AI</button></div>
            <div className="space-y-3 text-sm text-slate-700">
              <div className="p-3 rounded-xl bg-slate-50 border border-slate-200"><div className="font-medium text-slate-900 mb-1">Why call now</div><p>Acme’s import volume on SHA→LAX has surged (+18% QoQ). Likely evaluating new capacity and rates for peak.</p></div>
              <div className="p-3 rounded-xl bg-slate-50 border border-slate-200"><div className="font-medium text-slate-900 mb-1">Hook</div><p>Share lane‑level market benchmarks and a 3‑lane quote with value‑add (tariff estimator + guaranteed cut‑off slots).</p></div>
              <div className="p-3 rounded-xl bg-slate-50 border border-slate-200"><div className="font-medium text-slate-900 mb-1">Next Best Action</div><ul className="list-disc ml-5 space-y-1"><li>Send intro to Maya (VP Supply Chain) with SHA→LAX spot comp.</li><li>Book a 15‑min discovery with Derrick (Logistics Manager).</li><li>Prep RFP draft with 3 carriers and 2‑week trial lane.</li></ul></div>
            </div>
          </div>
          <div className="p-5 rounded-2xl bg-white border border-slate-100 shadow-sm">
            <div className="flex items-center justify-between mb-3"><div className="font-medium text-slate-900">RFP Quick Draft</div><span className="inline-flex items-center rounded-full bg-gray-100 text-gray-700 text-xs px-2 py-1">Quote Generator</span></div>
            <div className="grid grid-cols-2 gap-3 text-sm">
              <label className="grid gap-1"><span className="text-slate-600">Origin</span><input defaultValue={company.topRoutes?.[0]?.origin ?? 'CN SHA'} className="px-3 py-2 rounded-xl border border-slate-200"/></label>
              <label className="grid gap-1"><span className="text-slate-600">Destination</span><input defaultValue={company.topRoutes?.[0]?.dest ?? 'US LAX'} className="px-3 py-2 rounded-xl border border-slate-200"/></label>
              <label className="grid gap-1 col-span-2"><span className="text-slate-600">Commodities / HS</span><input placeholder="e.g., 8471.49; robotics modules" className="px-3 py-2 rounded-xl border border-slate-200"/></label>
            </div>
