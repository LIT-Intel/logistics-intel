...
import { checkFeatureAccess } from "@/auth/AuthProvider";
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100">
      <div className="flex min-h-screen">
        <aside
          className={`${collapsed ? "w-16" : "w-64"} transition-all duration-200 shrink-0 bg-gradient-to-b from-slate-800 via-slate-700 to-slate-900 shadow-2xl hidden md:flex md:flex-col`}
        >
            ...
            <div className="flex items-center gap-3 min-w-0">
              ...
              <nav className="hidden sm:flex items-center gap-2 text-sm text-gray-600 truncate">
                ...
                {breadcrumbs.slice(1).map((b, idx) => (
                  <span
                    key={idx}
                    className={`truncate ${idx === breadcrumbs.length - 2 ? 'font-medium text-gray-900' : ''}`}
                  >
                    {b.label}
                  </span>
                ))}
