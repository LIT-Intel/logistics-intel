...
export default function MxTradePanel({ companyName }) {
  ...
  // leftover slug decoration ('mx:' route ids, 'mx-' key prefixes).
  const name = useMemo(() => {
    let v = String(companyName || '').trim();
  ...
  const { data, isLoading } = useQuery({
    ...
    staleTime: 5 * 60 * 1000,
    queryFn: async () => {
      const { data: d, error } = await supabase.rpc('lit_mx_company_profile', { p_name: name });
