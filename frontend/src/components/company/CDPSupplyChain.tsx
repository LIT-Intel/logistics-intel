...
/* Lane intelligence (Central Intelligence Hub) presentation was extracted
 * to frontend/src/components/maps/LaneIntelligenceCard.tsx so the workspace
 * dashboard map can reuse the same purely-presentational card. This file owns
 * the useLaneShipmentIntel fetch and passes the resolved data in below. */
...
function TradeLanesMapDialog({
  ...
  // Central Intelligence Hub — BOL-level detail for the selected lane. The
  // hook is disabled (resolves null) until company + both country names are
  // present, so passing the (possibly null) selected pair's names is safe.
  const { data: selLaneIntel, isLoading: selLaneIntelLoading } =
  ...
  // EXACT series the Cadence chart uses. Used to give the selected lane's
  // trend true seasonality instead of the sparse ~50-BOL sample.
  ...
  // The selected lane's share of the real scope total. Applying this share to
  // the real monthly series yields the lane's true seasonal SHAPE scaled to
  // its size (sums back to the lane's reconciled volume) — the same
  // reconcile-to-Cadence approach already used for lane volume totals.
  ...
  const monthlyChartData = useMemo(() => {
    if (!selPair) return [];
    // Preferred: real seasonality (Cadence source) scaled to this lane.
        ...
        <div className="mt-3 border-t border-slate-100 pt-3">
          ...
          <div className="max-h-[46vh] overflow-y-auto pr-0.5">
            <LaneIntelligenceCard
              data={selLaneIntel ?? null}
              isLoading={selLaneIntelLoading}
              scope="company"
            />
          </div>
        </div>
