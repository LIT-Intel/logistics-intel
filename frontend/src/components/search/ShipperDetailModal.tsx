...
export default function ShipperDetailModal({
  ...
  const teus12m = useMemo(() => {
    if (!stats) return null;
    // Prefer aggregated TEUs from routes if present, otherwise from monthly series.
    if (Array.isArray(stats.routes) && stats.routes.length > 0) {
      const total = stats.routes.reduce((sum, route) => sum + (route.teu_12m ?? 0), 0);
      return Number.isFinite(total) && total > 0 ? total : null;
    }
    if (Array.isArray(monthlySeries) && monthlySeries.length > 0) {
      const total = monthlySeries.reduce((sum, item) => sum + (item.teu ?? 0), 0);
      return Number.isFinite(total) && total > 0 ? total : null;
    }
    return null;
  }, [stats, monthlySeries]);
  ...
  return (
    <Dialog open={open} onOpenChange={(next) => { if (!next) onClose(); }}>
      <DialogContent className="flex max-h-[90vh] w-full max-w-4xl flex-col overflow-hidden rounded-2xl bg-white p-0 shadow-2xl">
            ...
            <div className="flex items-center gap-2">
              {onSave && (
                <Button
                  ...
                  disabled={saving}
                  className="inline-flex items-center justify-center rounded-full bg-indigo-600 px-4 py-1.5 text-xs font-semibold text-white shadow-sm hover:bg-indigo-700 disabled:cursor-not-allowed disabled:opacity-60"
                >
        ...
        <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as "overview" | "kpis" | "shipments" | "contacts")}>
          <div className="border-b px-6">
            <TabsList className="flex gap-2 border-b border-transparent py-3">
              <TabsTrigger
                value="overview"
                className="rounded-full px-3 py-1.5 text-xs font-medium text-slate-600 data-[state=active]:bg-slate-900 data-[state=active]:text-white data-[state=active]:shadow-sm"
              >
                Overview
              </TabsTrigger>
              <TabsTrigger
          ...
          <div className="overflow-y-auto px-6 py-6 max-h-[calc(95vh-140px)] space-y-6">
            <TabsContent value="overview" className="space-y-6">
              <section className="grid grid-cols-1 gap-3 md:grid-cols-4">
                <KpiCard icon={<Ship className="h-5 w-5 text-indigo-500" />} label="Shipments (12m)" value={formatNumber(shipper.totalShipments)} />
                <KpiCard icon={<TrendingUp className="h-5 w-5 text-indigo-500" />} label="Last shipment" value={shipper.mostRecentShipment ?? "—"} />
                <KpiCard icon={<MapPin className="h-5 w-5 text-indigo-500" />} label="Top route" value={derivedTopRoute ?? "—"} />
                <KpiCard icon={<BarChart3 className="h-5 w-5 text-indigo-500" />} label="TEUs (12m)" value={teus12m != null ? formatNumber(teus12m) : "—"} />
              </section>
