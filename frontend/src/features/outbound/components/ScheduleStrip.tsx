...
export function ScheduleStrip({ steps, launching, anchor }: { steps: BuilderStep[]; launching?: boolean; anchor?: number }) {
  ...
  const noAnchor = anchor === undefined;

  return (
    <div
      className="flex shrink-0 flex-col border-b border-slate-200 bg-[#F8FAFC]"
    ...
    >
      {noAnchor ? (
        ...
        >
          ...
          <span>
            Times projected from now — pick a launch time to lock the schedule.
          </span>
        </div>
      ) : null}
