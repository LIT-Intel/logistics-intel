...
export default function WorkspaceLanesGlobe() {
  ...
  const highlightId = useMemo(() => {
    ...
    const wantFrom = String(highlightedLane.from).toLowerCase();
    const wantTo = String(highlightedLane.to).toLowerCase();
    return (
      sorted.find(
        (l) =>
          l.from_label.toLowerCase() === wantFrom &&
          l.to_label.toLowerCase() === wantTo,
      )?.key || null
    );
            ...
            {sorted.map((l, i) => {
              ...
              return (
                <button
                  ...
                  className={[
                    "flex w-full items-center gap-3 border-b border-slate-100 px-4 py-2.5 text-left last:border-b-0",
                    isActive
                      ? "border-l-2 border-l-blue-500 bg-blue-50/60"
                      : "border-l-2 border-l-transparent hover:bg-slate-50/60",
                  ].join(" ")}
                >
                  <span className="font-mono w-5 shrink-0 text-[10px] text-slate-400">
                    #{i + 1}
                  </span>
