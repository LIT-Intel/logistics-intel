...
// ── Pulse Coach v2 native types ───────────────────────────────────────
export type CoachCardV2 = {
  id: string;
  category: "subscription"|"campaign"|"reply"|"contact"|"company"|"task"|"system"|"support"|"opportunity";
...
export async function getPulseCoachV2(): Promise<PulseCoachV2Result> {
  ...
  if (error) {
    return { ok: false, mode: "proactive_sweep", cards: [], context_snapshot: null, error: String(error.message || "coach_v2_failed") };
  }
  return {
    ok: Boolean(data?.ok),
    mode: "proactive_sweep",
...
export async function askPulseCoach(question: string): Promise<PulseCoachAnswer> {
  ...
  if (error) {
    return { ok: false, answer_md: "I hit a snag — try again in a moment, or check the help center.", cta: null, error: String(error.message || "coach_answer_failed") };
  }
  return {
    ok: Boolean(data?.ok),
...
function mapCardToLegacyNudge(card: CoachCardV2): CoachNudge {
  const accent =
    card.priority === "critical" ? "#DC2626" :
    card.priority === "high" ? "#EA580C" :
    card.priority === "medium" ? "#2563EB" : "#64748B";
  return {
    id: card.id,
    title: card.title,
    body: card.body,
