Warning: truncated output (original token count: 60954)
Total output lines: 7288
...
function cacheRowToHit(idx: any, kpi: any | null): IyShipperHit {
  ...
  return {
    ...
    cu…10954 tokens truncated…/ Key-shape mismatch fix (Polish 4): ImportYeti returns keys like
...
export async function launchCampaign(
  ...
  | { ok: true; queued: number; skipped: number; restarted?: number }
  | { ok: false; error: string; restartable?: number; suppressed?: number }
> {
  ...
  try {
    const { data, error } = await supabase.functions.invoke("queue-campaign-recipients", {
      body: { campaign_id: campaignId, manual_emails: manualEmails, restart_completed: restartCompleted },
    });
    ...
    if (data?.ok) {
      return {
        ...
        restarted: Number(data.restarted ?? 0),
      };
    }
    return { ok: false, error: String(data?.error || "launch_failed"), restartable: Number(data?.restartable ?? 0), suppressed: Number(data?.suppressed ?? 0) };
  } catch (e: any) {
...
// Pulse Coach types + implementations moved to frontend/src/api/pulse.ts on
// 2026-05-30 as part of the api domain split. Re-exported here for backwards
// compatibility — existing imports from "@/lib/api" keep working. New code
// should import from "@/api/pulse" directly.
export type {
  CoachNudge,
  WorkspaceLane,
  PulseCoachResult,
