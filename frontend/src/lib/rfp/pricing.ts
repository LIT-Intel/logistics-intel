import type { RfpLane, RfpRate, RfpRateScope, RfpCharge, PricedLane, PricedLineItem, PricedResult } from '@/types/rfp';
...
export function priceLane(lane: RfpLane, rate: RfpRate): PricedLane {
  ...
  return { laneIndex: -1, mode: lane.mode, equipment: lane.equipment, charges: items, unitCost, annualCost };
}
...
export function priceAll(lanes: RfpLane[], rates: RfpRate[]): PricedResult {
  ...
  for (let i=0; i<lanes.length; i++) {
    const lane = lanes[i];
    const candidates = matchRate(lane, rates);
