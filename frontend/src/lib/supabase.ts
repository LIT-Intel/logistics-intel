...
function createMockSupabaseResponse(data: any[] = []) {
  return Promise.resolve({ data, error: null });
}

function createMockSingleResponse(data: any = null) {
  return Promise.resolve({ data, error: null });
}
...
function setLocalSavedCompanies(rows: any[]) {
...
export async function getSavedCompaniesFromSupabase(stage = 'prospect') {
  try {
    if (supabaseError) {
      ...
      const stored = getLocalSavedCompanies().filter((row: any) => row.stage === stage);
