...
r.post('/crm/companies', limiter, async (req, res, next) => {
  try {
    ...
    const upsert = await p.query(
      `INSERT INTO companies(name, website, plan, external_ref)
       ...
       RETURNING id`,
      [body.name, body.website ?? null, body.plan ?? null, body.external_ref ?? null]
    ).catch(async (_err) => {
      // If (name) is not unique, fallback to simple insert and ignore conflict requirement
      const ins = await p.query(
        `INSERT INTO companies(name, website, plan, external_ref) VALUES($1,$2,COALESCE($3,'Free'),$4) RETURNING id`,
        [body.name, body.website ?? null, body.plan ?? null, body.external_ref ?? null]
      );
      return ins;
    });
...
r.get('/crm/companies/:id', limiter, async (req, res, next) => {
  try {
    ...
    if (company.rowCount === 0) return res.status(404).json({ error: 'not_found' });
