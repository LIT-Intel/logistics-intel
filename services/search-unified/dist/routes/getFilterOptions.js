...
async function handleGet(req, res, next) {
    try {
        ...
        const payload = {
            modes: modesRows.map(r => r.mode).filter(Boolean),
            origin_countries: originsRows.map(r => r.code).filter(Boolean),
            destination_countries: destsRows.map(r => r.code).filter(Boolean),
            carriers: carriersRows.map(r => r.carrier).filter(Boolean),
            hs_prefixes: hsPrefixRows.map(r => r.hs_prefix).filter(Boolean),
        ...
        try {
            ...
            const payload = {
                modes: modes2Rows.map(r => r.mode).filter(Boolean),
                origin_countries: origins2Rows.map(r => r.code).filter(Boolean),
                destination_countries: dests2Rows.map(r => r.code).filter(Boolean),
                carriers: [],
                hs_prefixes: [],
                date_min: dateBounds2?.date_min ?? null,
                date_max: dateBounds2?.date_max ?? null,
                value_buckets: [
                    { min: 0, max: 10000 },
                    { min: 10000, max: 50000 },
                    { min: 50000, max: 100000 },
