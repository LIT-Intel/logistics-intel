...
type SearchRow = {
  total?: number | null;
  results?: SearchResultStruct[] | null;
};
...
function sanitizeArray(values?: unknown[], clampSize?: number): string[] {
...
export async function searchCompanies(input: SearchInput) {
  ...
  const resultRows = Array.isArray(rows) ? (rows as SearchRow[]) : [];
  ...
  const results = Array.isArray(row.results)
    ? row.results.map((entry) => ({
        ...
        top_routes: Array.isArray(entry?.top_routes)
          ? entry.top_routes.filter((route): route is string => typeof route === "string")
          : [],
        top_carriers: Array.isArray(entry?.top_carriers)
          ? entry.top_carriers.filter((carrier): carrier is string => typeof carrier === "string")
          : [],
      }))
    : [];
