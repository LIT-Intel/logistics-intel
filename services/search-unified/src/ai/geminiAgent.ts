...
export type LitGeminiInput = {
  ...
  command_center_state: Record<string, any> | null;
...
function buildPrompt(input: LitGeminiInput | Record<string, any>): string {
...
export async function runGeminiAgent(
  input: LitGeminiInput | Record<string, any>,
): Promise<any> {
  const ai = ensureClient();
  const prompt = buildPrompt(input);

  const maybeNextGen = ai as unknown as {
    models?: {
  ...
  try {
    ...
    throw new Error(
      `Gemini response could not be parsed as JSON: ${(err as Error).message}`,
    );
  }
}
